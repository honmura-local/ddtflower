...
CREATE_NEW_LESSON_MESSAGE 					 = '新規授業の作成に成功しました。';					//新規授業作成完了時に表示されるメッセージ
 ...
 * 引数　:createTagInstance:creator:クリエイトタグのインスタンス名
 * 返却値:なし
 * 作成日　:2015.07.31
 * 作成者　:T.Yamamoto
 */
function openMemberReservedConfirmDialog(creator) {
	//予約確認ダイアログを表示する処理
	$(DOT + DIALOG_CONTENT_RESERVED_LESSON_LIST).on(CLICK, DOT + LESSON_TABLE_RECORD, function(){
		//クリックした行の番号とデータを取得する
		var recordData = getClickTableRecordData(this, LESSON_TABLE, LESSON_TABLE_RECORD, creator);
		//残席の記号を取得する
		var restMarkNow = $(DOT + LESSON_TABLE_RECORD +':eq(' + (recordData.number) + ') td').eq(4).text();
		//残席が✕でないものでかつ、会員が受講できないようになっている授業(NFDなど)についてはクリックして予約確認ダイアログは開かない
		if (creator.json[LESSON_TABLE][TAG_TABLE][recordData.number][COLUMN_NAME_DEFAULT_USER_CLASSWORK_COST] && restMarkNow != '✕') {
			//予約する人が誰なのかを分かりやすくするために会員番号を送信する連想配列に入れる
			recordData.data[USER_ID] = creator.json.accountHeader.user_key.value;
			...
			if(creator.json[LESSON_TABLE][TAG_TABLE][recordData.number][COLUMN_NAME_USER_WORK_STATUS] != 1) {
				//ダイアログのタイトルをセットして予約日を分かりやすくする
 ...
 * 作成者　:T.Yamamoto
 */
function openAdminLessonDetailDialog(creator) {
	//管理者、授業一覧テーブルの行をクリックしたときにクリックした授業の詳細データをダイアログで開く
 ...
 * 作成者　:T.Yamamoto
 */
function openAdminNewLessonCreateDialog(creator) {
	//レコードをクリックして新規授業追加ダイアログを開くイベントを登録する
	$(DOT + DIALOG_CONTENT_ADMIN_LESSON_LIST).on(CLICK, DOT + ADMIN_LESSON_ADD_BUTTON, function(){
		//新規授業追加ダイアログに渡す変数を宣言しておく
		var sendObject = {};
		//日本語名の日付を渡すデータを入れる(DBの形式をそろえるためスラッシュはハイフンに置き換える)
		sendObject[LESSON_DATE] = $(DOT + DIALOG_CONTENT_ADMIN_LESSON_LIST)[0].instance.argumentObj.lessonDate.replace(/\//g,"-");
		//取得したテーブルの情報があればそれを新規作成ダイアログに渡す
		sendObject[TABLE_DATA] = creator.json.adminLessonDetailTable.table;
		//ダイアログのタイトルをセットして予約日を分かりやすくする
		dialogExOption[ADMIN_NEW_LESSON_CREATE][TITLE] = dialogExOption[ADMIN_LESSONLIST_DIALOG][TITLE];
		//新規授業追加ダイアログを作る
		var newLessonCreateDialog = new dialogEx(DIALOG_ADMIN_NEW_LESSON_CREATE, sendObject, dialogExOption[ADMIN_NEW_LESSON_CREATE]);
		newLessonCreateDialog.setCallbackClose(disappear);	//閉じるときのイベントを登録
...
function memberReservedConfirmDialogOkButtonFunc(sendObject) {
	if(!sendObject.user_work_status) {
		//DBにデータを挿入して予約処理をする。(creatorの対象は確認ダイアログのcreator)
	...
	$(DOT + DIALOG_CONTENT_MEMBER_RESERVED_CONFIRM).dialog(CLOSE);			//ダイアログを閉じる
}
...
function cancelLssonDialogDialogOkButton(sendObject) {
	...
	creator.tableReload(RESERVED_LESSON_TABLE);
	//ダイアログを閉じる
	$(DOT + DIALOG_CONTENT_CANCEL_LESSON).dialog(CLOSE);
}
 ...
 * 引数　:object: recordData: 行のデータ連想配列
 * 返却値:timeTableDayKey:時限のidデータ
 * 作成日　:2015.08.08
 * 作成者　:T.Yamamoto
 */
function getTimeTableDayKey(recordData) {
	//授業一覧のデータを長さを取得し、ループが終わる回数として使う
	var loopEndCount =$(DOT + DIALOG_CONTENT_ADMIN_NEW_LESSON_CREATE)[0].instance.argumentObj.tableData.length;
	...
	for(var loopStartCount = 0; loopStartCount < loopEndCount; loopStartCount++) {
		...
		if(recordData[COLUMN_NAME_TIMETABLE_KEY] == $(DOT + DIALOG_CONTENT_ADMIN_NEW_LESSON_CREATE)[0].instance.argumentObj.tableData[loopStartCount][COLUMN_NAME_TIMETABLE_KEY] 
			...
			//時限データを取得し、ループを終える
			timeTableDayKey = $(DOT + DIALOG_CONTENT_ADMIN_NEW_LESSON_CREATE)[0].instance.argumentObj.tableData[loopStartCount][COLUMN_NAME_TIME_TABLE_DAY_KEY];
			//ループを終わらせる
			break;
		}
	}
 ...
 * 作成日:2015.08.03
 */
function newLessonEntry(creator) {
	//新規授業追加ダイアログで入力された値を取得し、DBに値をinsertする時に使う
	var newLesoonData = getInputData(LESSON_DATA);
	//時限データが既にあればそれを取り出す
	var timeTableDayKey = getTimeTableDayKey(newLesoonData);
	//新しく授業データを作るために授業日を連想配列に入れる
	var lessonData = {
			lessonDate:$(DOT + DIALOG_CONTENT_ADMIN_NEW_LESSON_CREATE)[0].instance.argumentObj.lessonDate,
	...
	if(timeTableDayKey == "") {
		...
		creator.setDBdata(creator.json.insertTimeTableDay, sendReplaceQuery, '');
		//新規に授業のデータをDBに登録する
		creator.setDBdata(creator.json.newClassWork, sendReplaceQuery, CREATE_NEW_LESSON_MESSAGE);
	//予約する時限があった時にそれを使って新規授業を作成する
	} else {
		//すでにある時限データを使って授業データを作る
		creator.setDBdata(creator.json.normalInsertClasswork, sendReplaceQuery, CREATE_NEW_LESSON_MESSAGE);
	}
}
